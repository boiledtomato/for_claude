...
│   ├── otp/                           # TOTP の計算と取り込み（Android に依存しない）
...
認証アプリをいちいち開かずに済ませるための面。**他アプリの画面を埋め込んでいるのではない**
― Android には他アプリの UI を自分の画面に描かせる公開 API が無く、埋め込めるのは相手が
ウィジェットとして差し出したものだけ。アクセシビリティで画面を読む手段は、全アプリの画面
内容を取れる過大な権限で、認証アプリの画面はたいてい `FLAG_SECURE` で保護されている。
だからここでは**コードの計算そのものを持っている**（RFC 6238 の TOTP）。

### 計算

`core/otp/` に置いてあり、Android に依存しない:

- `Base32` ― RFC 4648。受け取りは緩くする（小文字・空白・ハイフン・末尾の `=` を落とす）。
  手で打つことも、QR から貼ることもあるため
- `Totp` ― HMAC（SHA1 / SHA256 / SHA512）、6〜8 桁、任意の周期
- `OtpUri` / `OtpMigration` ― 取り込み（下記）

`TotpTest` が **RFC 6238 Appendix B の試験値**をそのまま固定している。コードは「合っているか」
を目視で確かめられない類のもので、1 桁ずれてもサイト側が悪いようにしか見えない。

**端末の時計がずれると全部外れる。** 認証側は普通 ±1 ステップを許すので数十秒なら通るが、
分単位でずれた端末では何をしても合わない。ここで勝手に補正はしない ― ずれていることに
気付けなくなる。

### 保管

`data/otp/OtpVault`。ほかの設定（DataStore の JSON）とは**別のファイル**に、Android Keystore
の鍵で AES-GCM 暗号化して置く。あちらは配置を変えるたびに書き換わり、丸ごとコピーされる
機会も多い。

- **発行元やアカウント名も含めてファイルごと暗号化する。** どのサービスを使っているかも
  知られたくない情報なので、鍵だけ隠しても足りない
- **画面ロック中は復号できない**（`setUnlockedDeviceRequired`）。コードを見るのは画面が
  点いているときだけなので、これで失うものは無い
- 鍵をメモリに持つのは**この面を開いている間だけ**。離れると落とす
- バックアップと端末の引っ越しから除外（`data_extraction_rules.xml` / `backup_rules.xml`）。
  鍵は Keystore にあって端末から出ないので、ファイルだけ移しても復号できない

**「読めなかった」と「空だった」を絶対に混ぜない。** 混ぜると、読めなかったあとに 1 件
追加した時点で、残っていた保管庫を空で上書きする。`OtpVault.Result` が `Ready` / `Locked` /
`Unreadable` を返し分け、`Unreadable` の間は**書き込みを止める**（画面には「読めません。
作り直すまで何も足せません」と出し、捨てるかどうかは利用者が決める）。認証コードは失うと
サイト側の復旧手続きに行くしかないので、ここは黙って先へ進ませない。

### 取り込み

**まず QR を読む（Auth → Scan）。** ほかの認証アプリから鍵を取り出す方法は無いので、
移行の実作業は「各サービスの二要素認証設定で認証アプリを追加し直し、出てきた QR を読む」
ことになる。1 枚読んでも閉じないので、何十件でも続けて読める。読み取りは端末の中だけで
完結し（CameraX + zxing core、Play 開発者サービスに依存しない）、絵は保存しない。
カメラを断られても、貼り付けと手入力で登録できる。

`QrLuminance` に切り出してあるのは、**端末が無いと確かめられない部分を減らす**ため。
実機でしか出ない不具合はたいてい行の余白（`rowStride`）と向きの取り違えで、どちらも
純粋な計算なので `QrLuminanceTest` で固定してある。

**形式を選ばせない。** 貼られた文字列の中から `otpauth://totp/…` と
`otpauth-migration://offline?data=…`（Google Authenticator の書き出し）を拾う。1 件でも、
行で並べたものでも、JSON に埋まっていても同じ操作になる。1 件が壊れていても、読めたものは
入れる。手入力（発行元・アカウント・base32 の鍵）も残してある。

`otpauth-migration://` は**公開仕様ではない**。パーサーは広く知られているスキーマに基づく
実装で、`OtpMigrationTest` が固定しているのは**合成データ**であって本物の書き出しではない。
だから取り込み画面は、保存前に読み取った中身（発行元・アカウント・桁数・周期）を必ず一覧で
見せる。**元のアプリを消す前に、両方のコードが一致することを必ず確かめること。**

カウンタ式（HOTP）は取り込まない。ずれた時点で使えなくなり、こちら側では直せない。

### 表示について決めたこと

コードの表示に生体認証は**挟まない**（利用者の判断）。したがって上の暗号化が守るのは
「端末から吸い出したファイル」であって、「端末を手にした人」ではない。そのぶん、面を開いて
いる間だけ `FLAG_SECURE` を立てて画面の撮影と録画を止める（**この面はスクリーンショットが
撮れない**）。コピーは Android 13 以降「機密」の印を付ける ― 付けないと貼り付け候補の
プレビューに 6 桁がそのまま出る。

**デバッグ署名鍵をリポジトリに公開している点は、この機能に限って重い。** 同じパッケージ名・
同じ鍵で署名した APK は「更新」として入り込め、アプリのデータ領域と、UID に紐づく Keystore
の鍵に到達できる。本気で使うなら、リリース用の鍵を別に用意すること。

### ホーム画面のウィジェット（アカウント 1 件 = 1 枚）

コンソールを開かずに済ませたいとき用。**Widgets → Add widget** の一覧に
`Authentication code` があり、置くと「どの登録を映すか」を選ぶ画面が出る。出したい
アカウントの数だけ置く（1 枚 = 1 件）。標準の大きさは 2×2、横幅は 1〜4 列に伸ばせる。

**先に Auth ペインへの登録が要る。** ウィジェットが映すのはこの保管庫の中身なので、
1 件も入っていなければ `No codes yet` と出て何も置けない。その画面の
**Add a code in Auth** を押すとコンソールの Auth ペインまで飛ぶ（ウィジェットの配置は
取り消しになる ― 中身の無い枠をホーム画面に残さない）。登録してから置き直す。

- **常時表示しない。** 伏せ字（`••• •••`）で置いてあり、**タップして 30 秒だけ**出る。
  ホーム画面は誰の目にも入る場所なので、ここだけは既定を「見えない」側に倒している
- **残り秒は `Chronometer` に数えさせる。** こちらから毎秒描き直すと、1 回の表示で 30 回
  プロセスが起きる
- 30 秒の窓の途中でコードが切り替わったら描き直す。窓の終わりだけを見ていると、切り替わった
  古いコードを出し続ける（認証側は普通 1 つ前まで通すので、「たまに通らない」形で出る）。
  判定は `TotpReveal`、`TotpRevealTest` が固定している
- **システムの定期更新（`updatePeriodMillis`）は使えない。** 最短で 30 分、30 秒ごとの
  コードには桁が違う。更新はタップ起点で、アラーム 1 本を使い回す。正確なアラームの権限は
  取らない ― 見えている間はホーム画面が前面なので、数百ミリ秒しか遅れない
- 画面ロック中は鍵が使えないので（`setUnlockedDeviceRequired`）、`Unlock the phone to
  show it` と出して伏せたままにする
- 保管庫から消した登録を映していたウィジェットは `No longer in the vault` になる。名前を
  変えたときは、次のタップを待たずにその場で追従する（`OtpWidgetRenderer.syncBindings`）
- タップとアラームの受け口は提供元と**別のレシーバー**にして `exported="false"` にしてある。
  ウィジェットの提供元は公開が必須なので、そこに自前の action を足すと、他アプリから
  「表示」を投げてホーム画面にコードを出させられる

**ウィジェットに `FLAG_SECURE` は効かない。** あれはウィンドウ単位の設定で、ホーム画面の
ウィンドウはランチャーのもの。つまり Auth ペインと違い、**コードが出ている間のホーム画面は
スクリーンショットに写る**。30 秒で自動的に伏せるのはそのためでもある。

保管庫に入っているのは鍵とコードだけで、**ウィジェットに割り当てた発行元・アカウント名は
素のまま**（`OtpWidgetStore`）持っている。ホーム画面に置いた時点でその名前は端末を覗いた
誰にでも見えていること、再起動直後など保管庫を読めない場面でも枠が空にならないことを取った
妥協で、鍵とコードの扱いは変えていない。

### Microsoft Authenticator などからは持ってこられない

**他アプリが持っている鍵は読めない。** Microsoft Authenticator の登録はあのアプリの
サンドボックスの中にあり、書き出しの口も、他アプリ向けの API も無い（Android の画面を
他アプリに埋め込む方法も無いので、あの画面をここに出すこともできない）。そのクラウド
バックアップも、戻せる先は Microsoft Authenticator だけで、こちらからは読めない。

したがって移行は**サービス側からやり直す**。各サービスの二要素認証設定で「認証アプリを
追加」を選び、出てきた QR を Auth → Scan で読む。元のアプリの登録はコードが一致することを
確かめるまで消さないこと。

**そもそも移せないものがある。**

- **職場・学校（Entra ID）アカウント**のプッシュ承認（番号を選ぶ形式）は Microsoft 独自の
  やりとりで、TOTP ではない。こちらでは再現できない
- 同じ職場アカウントが出す 6 桁も、鍵を発行しているのが Microsoft 側なので取り出せない
- 会社の条件付きアクセスが認証アプリを指定している場合、規定として移してはいけないことも
  ある

つまり ZLauncher に入れられるのは、**サービス側が QR で鍵を配ってくれる登録**
（GitHub、AWS、Google など、Microsoft Authenticator で言えば「その他のアカウント」）。
Microsoft のアカウントはあちらのアプリを使い続けることになる。
