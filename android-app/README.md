...
`WidgetPlacement.heightDp` を後から変えられる。ヘッダの「Size」を押すとサイズ変更モードに入り、
±20dp のボタンと、下端のつまみのドラッグで調整できる。範囲は 72〜560dp
（`WidgetPlacement.clampHeight`）。提供元の申告する `minHeight` より小さくしてよい —
多くのウィジェットは `resizeMode` で縮小を許しており、時計や 1 行のものは 80dp でも読める。
「Reset」でその `minHeight` に戻す。

2 つ、外すと動かなくなる点:

- **つまみはサイズ変更モードのときだけ出す。** 常時出すと、ホームを縦にスクロールしたつもりが
  高さを変えてしまう
- **`AppWidgetHostView.updateAppWidgetSize()` を呼ぶ。** 呼ばないとウィジェットは初回に選んだ
  レイアウトのまま引き伸ばされる。幅は `BoxWithConstraints` の実測値を渡す。API 31 以降は
  `SizeF` のリストを取る版が正で、4 引数版は非推奨
