...
折りたたみ**で並べる。選ぶ単位は小項目。大項目は 28、小項目は 112 あるので、
...
**`Advanced Security` 大項目は取り込み時点で落とす。** CSV 自身が「このカテゴリーは
ATP ポリシーにのみ適用される」と注記しているとおり、閲覧内容の分類ではなく脅威検知の
区分であり、ランチャーのフォルダとしては意味を成さない（13 件）。
...
通知は Android 13 以降だと実行時の許可が要るが、許可が無くても帯には出るので気付ける。プリセットで作ったものも普通のカテゴリーなので、
