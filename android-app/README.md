...
| カード | 内容 | 既定 | タップ |
|---|---|---|---|
| ヘルスチェック | 5 項目を掃引アニメーションで順に点検、9 秒ごとに再実行 | 表示 | 即時再点検 |
| トラフィック | 下り / 上りの実測レート＋直近 40 秒のスパークライン | 表示 | — |
| バッテリー | 残量・充電状態・温度 | 表示 | 電池使用量の設定 |
| ストレージ | 使用量 / 空き | 表示 | ストレージ設定 |
| メモリ | 使用量 / 全体 | 表示 | — |
| ネットワーク | 回線種別・VPN・推定下り速度 | 表示 | ネットワーク設定 |
| バッテリー推移 | セッション中の残量スパークライン | 表示 | — |
| 時刻 | 秒まで動く時計・日付・稼働時間 | 表示 | — |
| 通信量（起動から） | 受信 / 送信の累計、モバイルと Wi-Fi の内訳 | 表示 | — |
| 最近追加したアプリ | 直近 3 件、タップで起動 | 表示 | — |
| 連続稼働 | 最終起動からの経過 | 非表示 | — |
| アプリ | 起動可能なアプリ数（仕事用を含む） | 非表示 | アプリ設定 |
| 音量と明るさ | メディア / 着信 / 画面の明るさ | 非表示 | — |
| 端末 | モデル・Android バージョン・解像度・CPU コア数 | 非表示 | — |

非表示のカードは編集モードの「非表示のカード」から出せる。

追加の権限が要るもの（アプリ別通信量、利用時間、Wi-Fi の SSID）は入れていない。使うのは
`ACCESS_NETWORK_STATE`（通常権限）だけで、実行時の許可ダイアログは出ない。トラフィックは
...
追加ダイアログには **Zscaler の URL カテゴリーの切り口に沿ったプリセット**（`PRESET_CATEGORIES`、
20 種）を並べてあり、まとめて選んで作れる。プリセットで作ったものも普通のカテゴリーなので、
