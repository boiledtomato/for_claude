...
他アプリに埋め込む方法も無いので、あの画面をここに出すこともできない）。ここのウィジェットに
出せるのは**この保管庫に入れ直した登録**だけで、それには各サービスの二要素認証の設定画面で
QR を出し直す（＝登録し直す）必要がある。

会社のアカウントは、そもそも移せないことがある。Entra ID の条件付きアクセスで
Microsoft Authenticator を指定している場合や、プッシュ承認（コードではない）の場合は、
あちらのアプリを使い続けるしかない。
