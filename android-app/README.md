...
- **長押しだけを横取りする**（`interceptLongPress`）。ウィジェットは提供元アプリのビュー
  なので、膜をかぶせると普通に使えなくなる。Initial パスで様子だけ見て、長押しの時間だけ
  指が止まっていたらそこから先を自分のものにする ― 早く離せば何も消費しないので、
  タップもスクロールも素通りする。ホーム画面のランチャーが普通にやっていること
