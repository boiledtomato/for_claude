...
- **Colour tokens are theme-aware.** `ZColors.X` reads `LocalZColors` (a `ZColorScheme`), so call sites are unchanged but the values differ between the dark and light schemes — and they cannot be read outside composition (a `DrawScope` must be handed the colour). The light scheme is taken from the Zscaler Experience Center console (nav blue `#1D56CE`, white page, `#E3E8EF` rules). The mode (`ThemeMode`, default SYSTEM) is persisted and picked in the **Appearance** dialog on the rail's bottom button (it used to cycle on tap); `MainActivity` re-applies `enableEdgeToEdge` so the status bar icons follow, since the user can pin light while the device is dark
...
- **The widget picker lists both profiles.** `getInstalledProvidersForProfile` returns one profile's providers despite the plural name, so it is called per profile and the results concatenated; binding uses the `bindAppWidgetIdIfAllowed` overload that takes a `UserHandle`, and the consent intent carries `EXTRA_APPWIDGET_PROVIDER_PROFILE` — without either, both assume the caller's user and a work-profile widget cannot be placed. An app installed in both profiles exposes the **identical** `ComponentName`, so the row key mixes in the user; keyed on the provider alone, selecting one profile's widget marks the other selected too. The rows are told apart by a badged icon, a trailing `Work`, and the search term `work`; `ProviderItemProfileTest` pins it, since none of this shows up on a device without a work profile
...
- Placed widgets live on **sheets you flick between** (`HorizontalPager` in `WidgetsPane`): one column that keeps growing downward put the widgets at the bottom out of reach. Sheets are added with an **`Add sheet` button in the pane**, next to the page dots at the bottom — not by flicking onto a trailing add-page, and not from the header, which already carries `Layout` and `Add widget` and has under 300dp to work with once the 84dp rail is subtracted. Pressing it creates a sheet and pages to it, using **the sheet count captured at press time** as the destination: the `sheets` list is still the old one at that moment, so reading `lastIndex` would page back to the sheet you are already on. The button works from an empty sheet too. A sheet left empty is dropped when you flick off it — `pruneEmptySheets(keep)` always keeps the sheet you are standing on, or a sheet would vanish the moment it was created — and two rules keep that cleanup from eating the sheet you just made. First, **the settle effect must not key on `sheets`**: keyed on it, every add rebuilt the effect and `snapshotFlow` immediately re-emitted the page the pager had not left yet, so the prune deleted the new empty sheet within a frame (on device: pressing Add added nothing and went nowhere). It reads the list through `rememberUpdatedState` and lives as long as the `pagerState`. Second, **the settle that comes from the add skips the prune once**, so adding from an empty sheet does not delete the one behind it and leave the count unchanged. `WidgetSheetsTest` pins the hazard by asserting what a stale `keep` does. The English "Add a widget sheet?" Yes/No prompt appears **only when there are no sheets and no widgets at all**, and a No is not asked again while the pane stays open. Paging is disabled in Layout mode (`userScrollEnabled = !editing`) because the width grab handle drags along the same axis, and `beyondViewportPageCount = 0` keeps off-screen provider views from rendering. `WidgetPlacement.sheetId` holds the assignment; **pre-sheets data has a blank id** and is folded onto the first sheet on load by `WidgetSheets.normalize`. Reordering (`moveTo`) and the `◀`/`▶` buttons are sheet-scoped — the sheet-local index is mapped back onto the global list's slots — and all of that arithmetic is pure in `WidgetSheets`, pinned by `WidgetSheetsTest`
- Within a sheet, widgets flow into a **4-column grid** (`WidgetFlow.rows`): each carries a `widthSpan` (1-4) and `heightDp`, and only a widget that does not fit the remaining columns starts a new row, so two narrow ones share a row. A gap is left as a gap — pulling a later widget up would move something the user placed. The starting span comes from the provider's own `minWidth`, which is declared as `70dp × cells − 30dp`: `spanForWidthDp` adds the 30 back before dividing, or every 2×1 widget collapses to one column (`WidgetFlowTest` pins it). Sizing happens only in the pane's **Layout** mode, on the selected widget, from a bar at the top of the pane — a widget can be one column (~60dp) wide, so controls inside its own frame would always overflow — plus grab handles on the selected widget's right edge (width, one column per half-column of travel) and bottom edge (height). In Layout mode a veil covers the widget so the provider app never sees the touch; selection taps and the long-press drag are handled by the veil, not the widget. Height is 72-560dp (`WidgetPlacement.clampHeight`) via the bar or a handle shown only while selected; `updateAppWidgetSize` must be called with the measured width or the widget keeps its first-chosen layout **Widgets are also removed in bulk by long-pressing one** — that enters a removal mode (pink border, jiggle, top-left `−`, same shapes as the category one), further widgets are added by tapping, and a floating `Remove N` sits at the **bottom right** (a full-width bar would permanently hide the last row; the list's bottom padding grows to match). The long press is intercepted **in the View layer, not in Compose** (`ui/widgets/LongPressHostLayout.kt`, a FrameLayout hosting the `AppWidgetHostView`): a veil over an un-edited widget would stop the provider app working, and a Compose gesture missed the press on a real device because the widget's own View eats the stream — `onInterceptTouchEvent` always runs before children get anything, which is how every launcher does it. It claims the gesture only after the finger holds still past the long-press timeout, and drops it when the child calls `requestDisallowInterceptTouchEvent` (a widget scrolling inside itself). `LongPressHostLayoutTest` dispatches **real `MotionEvent`s through an attached ViewGroup** — Compose's test input enters at Compose's own entry point and cannot reproduce View-layer dispatch, which is exactly how the first broken version passed its tests; the view also has to be attached to an Activity or `postDelayed` never runs. The deletion is one write (`WidgetRepository.remove(Collection)`), since writing per widget re-renders the list once per removal
- **Colour adjustment is a user setting.** The rail's bottom button opens **Appearance**: the light/dark mode plus a saturation (0-1.6x) and a brightness (0.6-1.4x) slider, persisted as `ColorAdjust` and applied to every token by `ZColorScheme.adjusted()`. Hue is never moved (it would break the identity/chart colour pairing), and **text is not simply multiplied**: surfaces are computed first, then each text token's contrast is re-measured against the worst of them and its *lightness alone* pushed away until it clears its floor (4.5 body / 4.0 secondary / 3.0 dim and the danger bar, which is large text and is only 3.53:1 in the stock dark palette anyway). The maths lives in `core/color/ColorMath.kt` (Android-free: HSL, luminance, contrast) and `ColorAdjustmentsTest` sweeps **both schemes across the whole slider range** rather than trusting a look — the only way this feature breaks is surface and text drifting to the same side. A neutral setting short-circuits and returns the measured stock palette untouched. Sliders commit to DataStore on release, not per pixel
...
- The category colour swatches are **one `CategoryColorPicker`** (`ConsoleDialogs.kt`) used by both the add dialog and rename/recolor. They used to be two implementations and the rename one was a plain `Row`: 15 swatches at 28dp do not fit on one line, so everything past the dialog's width was simply clipped and the colours looked fewer when changing than when adding. The `FlowRow` wrap is what makes all 15 reachable, not a cosmetic choice
