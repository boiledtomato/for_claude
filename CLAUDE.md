...
- The only OTP-related thing left is a widget that **opens Microsoft Authenticator** (`ui/widgets/authenticator/`): the provider draws the other app's icon, its localized label and `Tap to open`, and a `PendingIntent` launches it; `Not installed` when it is absent. Another app's TOTP seeds can never be read (no export, no API, no way to embed another app's UI), so nothing more than a launch shortcut is possible here — do not add anything that implies otherwise. Its background and border are shapes in `ImageView`s tinted with `setColorFilter`, so the widget follows the colour adjustment (a `-night` resource would ignore a user who pinned a mode). `updatePeriodMillis` is 0; `MainActivity.onStart` redraws it, which covers both a theme change and the other app being installed or removed. **The launcher's own Auth pane, TOTP vault, per-account code widget and QR scanner were removed at the user's request** — do not resurrect them (and the CameraX/zxing dependencies that came with the scanner are gone too)
