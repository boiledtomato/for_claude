...
- The console's **Auth pane** computes TOTP codes itself (`core/otp/`, RFC 6238) — no app can embed another app's screen on Android, so hosting the authenticator's UI was never an option. `TotpTest` pins the RFC 6238 Appendix B vectors. Secrets live in `data/otp/OtpVault`: a separate file (never the shared prefs blob), AES-GCM under an Android Keystore key with `setUnlockedDeviceRequired`, excluded from cloud backup and device transfer, decrypted only while the pane is open. **`OtpVault.Result` distinguishes `Unreadable` from empty and writes are blocked while unreadable** — treating "cannot decrypt" as "no entries" would overwrite the surviving vault on the next add, and lost TOTP secrets mean account-recovery flows. Import is QR-first (Auth → Scan: CameraX + `zxing core`, no Play services, the dialog stays open so dozens of accounts can be re-enrolled in one pass; `QrLuminance` holds the two things that only break on a real device — `rowStride` packing and frame rotation — as pure functions pinned by `QrLuminanceTest`, and the camera permission is optional). Pasting also works and extracts every `otpauth://` plus `otpauth-migration://` (Google Authenticator export, an undocumented protobuf whose parser is pinned only against synthetic data — the dialog therefore shows what it parsed before saving). Display is **not** behind biometrics (the user's explicit choice), so the encryption protects the file, not the person holding the phone; the pane sets `FLAG_SECURE` while open, and so does each dialog (a Compose `Dialog` is its own window, so the pane's flag does not reach the paste field where the secret is visible) and marks copies sensitive on API 33+. The committed debug keystore matters more here than anywhere else: a same-signature "update" inherits the data dir and the UID-scoped Keystore key, so a release key is a precondition for trusting this with real accounts
- Codes can also go on the home screen: an `AppWidgetProvider` (`ui/widgets/totp/`) with **one widget per account**, bound to a vault entry by a configure activity. It is masked by default and **tap-reveals for 30 seconds** — `updatePeriodMillis` has a 30-minute floor, so refreshes are tap-driven off a single reused alarm, and a reveal that outlives its code re-renders at the period boundary (`TotpReveal`, pinned by `TotpRevealTest`); the remaining seconds are counted by a `Chronometer` inside the RemoteViews rather than by redrawing every second. **`FLAG_SECURE` cannot cover a widget** (it is a per-window flag and the home screen is the launcher's window), which is the other reason for the 30-second auto-hide. Taps and alarms land on a *separate* `exported="false"` receiver: a widget provider must be exported, so sharing it would let any app force a code onto the screen. `OtpWidgetStore` keeps the issuer/account label in plain prefs — a deliberate, documented trade-off (the label is on the home screen anyway; the secret stays in the vault). Another app's seeds (Microsoft Authenticator, etc.) can never be read — no export, no API, no way to embed another app's UI — so each account has to be re-enrolled here
