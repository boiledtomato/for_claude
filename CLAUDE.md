...
- The console's Insights pane is **Web Insights**: per-category traffic logs over 1H/24H/7D from `NetworkStatsManager` (`data/insights/`), shown as three views — Chart (stacked-by-category timeline, connection/direction splits), Apps (ranked, with inline series), Log (raw rows) — searchable, filterable by time bucket and category, with tap-through detail for a category or app, and exportable as CSV via FileProvider. It needs usage access (`PACKAGE_USAGE_STATS`, signature|appop — granted in Settings, never by a runtime dialog); everything else works without it. URLs/hostnames are not obtainable without running a VPN service and are deliberately out of scope
- Chart fills use `ZColors.CategoryChartColors`, a darker sibling of the identity `CategoryColors` (hue drift <=8 degrees per slot) validated against the card surface: normal-vision min deltaE 18.2, CVD min 10.1, contrast >=3:1 on all pairs. Never eyeball a palette change here — re-run the validation. At most 5 categories get a hue; the rest fold into a neutral `Other`
- Console rail categories are built from Zscaler's predefined URL category CSV, bundled at `assets/zscaler_url_categories.csv` and re-fetched every 90 days by `CatalogSyncWorker` (`data/catalog/`). The picker groups sub-categories under collapsible super-category headers. The `Advanced Security` super-category is ATP-only and is dropped at parse time. The CSV URL carries a revision date so it cannot be hardcoded — the updater reads the help article via `zapi/fetch-data` and extracts the current `.csv` link. Diffs are never applied silently (the CSV has no stable ID column, so renames are inferred); the user reviews them. `UrlCategoryCsvTest` parses the real bundled file — keep it passing.
- UI strings are English throughout. Console cards are drag-reorderable at all times — the Layout button only reveals hide/resize chrome. All motion timings live in `core/designsystem/Motion.kt` (`ZMotion`) and every tappable uses `springyClick`
