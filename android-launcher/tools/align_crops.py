...
倍率と位置は正規化相互相関の総当たりで求める（数十秒）。

    python3 tools/align_crops.py full.png out.png --crops a.png b.png --scale 1.6 4.0
"""
import argparse
import numpy as np
...
def main():
    ...
    plate = Image.open(args.full)
    if plate.mode == "RGBA":
        plate = plate.crop(plate.getchannel("A").getbbox())
    ...
    factor = max(f[1] for f in fits)
    base = plate.resize((round(plate.width * factor), round(plate.height * factor)), Image.LANCZOS)
    base = base.filter(ImageFilter.UnsharpMask(radius=2.0, percent=args.sharpen, threshold=3))
    for path, s, x, y in fits:
        crop = Image.open(path).convert("RGB")
        c = crop.resize((round(crop.width / s * factor), round(crop.height / s * factor)), Image.LANCZOS)
